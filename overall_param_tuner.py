...
def test_10_times(is_regression):
    ...
    for i in range(10):
        ### for weight training
        dat_train = prepare_dataset(False, False, {}, False, 20)
        splited_data = dat_train[0], dat_train[1], dat_train[2], dat_train[3], dat_train[4], \
            dat_train[5], dat_train[6], dat_train[7], dat_train[8], dat_train[9]
        ...
        acc=train(is_regression=is_regression, dense_level=5, dropout_rate=0.15, epochs=40, batch_size=64, optimizer='adam',
            splited_dataset=splited_data, shut_up=True, class_weight={1: 0.3139914163090129, 2: 0.12482937482937483, 3: 0.3001805350402101, 0: 1.0},
             fn_prefix=str(i) + '_', save_xy=True)
        print(' acc = %.3f' %  (acc,))
        accs.append(acc)
