...
if __name__ == '__main__':
    ...
    for filter_cate in cates:
        ...
        for x in aial_test:
            ...
            # print('real',cates[real_cate_index])
            # print('pred',pred_tuple[0])
            origin_result = pred_tuple[0]

            #pred flipped
            # print('pred flipped')
            normed_img_flipped = np.fliplr(normed_img)
            pred = m.predict(np.array([normed_img_flipped]))
            #show predict stats
            cate_index = np.argmax(pred[0])
            # print('pred cate', cate_index)
            pred_tuple = []
            ...
            # print('real',cates[real_cate_index])
            # print('pred',pred_tuple[0])
            flipped_result = pred_tuple[0]

            # plt.imshow(normed_img)
            # plt.show()
            # plt.imshow(normed_img_flipped)
            # plt.show()

            print(origin_result, flipped_result)
            result_d[app_id] = (origin_result, flipped_result)

    save_pickle(result_d, 'preds_origin_vs_flipped.obj')
