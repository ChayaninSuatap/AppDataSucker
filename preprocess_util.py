...
def prep_rating_category_scamount():
    conn = db_util.connect_db()
    app_ids_and_labels = []
    dat=conn.execute('select app_id, rating, category, screenshots_amount  from app_data')
    for x in dat:
        if x[1] != None and x[2] != None and x[3] != None:
            app_ids_and_labels.append( (x[0], x[1], x[2], x[3])) 

    #rating str to float
    for i in range(len(app_ids_and_labels)):
        app_id , rating, cate, scamount = app_ids_and_labels[i]
        rating = float(app_ids_and_labels[i][1])
        scamount = float(app_ids_and_labels[i][3])
        app_ids_and_labels[i] = app_id, rating, cate, scamount
    
    #prep category
    for i,x in enumerate(app_ids_and_labels):
        onehot = _extract_category(x[2])
        app_ids_and_labels[i] = x[0], x[1], onehot, x[3]
